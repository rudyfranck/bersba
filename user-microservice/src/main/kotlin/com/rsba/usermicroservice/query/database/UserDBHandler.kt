...
object UserDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata? = null): MutableList<User> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata? = null): Optional<User> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(jsonHandler.decodeFromString<List<User>>("""$json""").firstOrNull())
            } else {
                Optional.empty()
            }
        } else {
