...
object AT_DBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata?): MutableList<AnalysisTemplate> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata?): Optional<AnalysisTemplate> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(jsonHandler.decodeFromString<List<AnalysisTemplate>>("""$json""").firstOrNull())
            } else {
                Optional.empty()
            }
        } else {
