...
object FeedbackArticleDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata? = null): List<FeedbackArticle> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata? = null): Optional<FeedbackArticle> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<FeedbackArticle>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
