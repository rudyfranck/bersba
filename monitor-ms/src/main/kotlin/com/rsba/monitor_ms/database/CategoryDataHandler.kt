...
class CategoryDataHandler(private val logger: KLogger) {
    ...
    fun all(row: Row?, meta: RowMetadata?): MutableList<CategoryOfItemInOrder> = try {
    ...
    fun one(row: Row?, meta: RowMetadata?): Optional<CategoryOfItemInOrder> = try {
        ...
        if (row != null) {
            ...
            if (json != null) {
                ...
                }.decodeFromString<List<CategoryOfItemInOrder>>("""$json""").firstOrNull())
            } else {
                Optional.empty()
            }
        } else {
