...
object TaskDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata? = null): List<Task> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata? = null): Optional<Task> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<Task>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
