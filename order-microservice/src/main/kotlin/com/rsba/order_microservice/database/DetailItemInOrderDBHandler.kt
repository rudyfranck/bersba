...
object DetailItemInOrderDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata? = null): List<DetailItemInOrder> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata? = null): Optional<DetailItemInOrder> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<DetailItemInOrder>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
