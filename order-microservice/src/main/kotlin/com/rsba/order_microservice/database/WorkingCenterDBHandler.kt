...
import com.rsba.order_microservice.domain.model.WorkingCenter
...
object WorkingCenterDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata?): List<WorkingCenter> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata?): Optional<WorkingCenter> = try {
        if (row != null) {
            ...
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<WorkingCenter>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
