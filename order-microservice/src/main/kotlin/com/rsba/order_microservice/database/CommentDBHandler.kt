...
object CommentDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata?): List<Comment> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata?): Optional<Comment> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<Comment>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
