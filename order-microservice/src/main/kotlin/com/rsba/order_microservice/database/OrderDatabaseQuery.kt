...
import com.rsba.order_microservice.domain.input.AddCategoryInOrderInput
import  com.rsba.order_microservice.domain.input.CreateOrderInput
import com.rsba.order_microservice.domain.input.EditCategoryOfOrderInput
import com.rsba.order_microservice.domain.input.EditOrderInput
...
class OrderDatabaseQuery {
    ...
    fun onRetrieveAllOrder(first: Int, after: UUID?, token: UUID) =
        "SELECT on_retrieve_orders('$first', ${after?.let { "'$it'" }},'$token')"
    ...
    fun onRetrieveCategoriesInOrder(input: UUID) =
        "SELECT on_retrieve_categories_by_order_id_post_mvp('$input')"

    fun onRetrieveItemsInOrder(orderId: UUID, categoryId: UUID) =
        "SELECT on_retrieve_items_by_order_id('$orderId', '$categoryId')"
