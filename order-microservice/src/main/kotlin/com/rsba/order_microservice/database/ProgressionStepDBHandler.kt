...
object ProgressionStepDBHandler {
    ...
    fun all(row: Row?): List<ProgressionStep> = try {
        if (row != null) {
    ...
    fun one(row: Row?): Optional<ProgressionStep> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<ProgressionStep>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
