...
import com.rsba.order_microservice.domain.model.Department
...
object DepartmentDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata?): List<Department> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata?): Optional<Department> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<Department>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
