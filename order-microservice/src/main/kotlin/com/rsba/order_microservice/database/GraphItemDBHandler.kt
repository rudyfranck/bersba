...
object GraphItemDBHandler {
    ...
    fun all(row: Row?, meta: RowMetadata? = null): List<Item.GraphItem> = try {
        if (row != null) {
    ...
    fun one(row: Row?, meta: RowMetadata? = null): Optional<Item.GraphItem> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<Item.GraphItem>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
