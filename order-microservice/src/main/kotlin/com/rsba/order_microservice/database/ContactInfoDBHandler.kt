...
import com.rsba.order_microservice.domain.model.ContactInfo
import com.rsba.order_microservice.domain.model.PersonalInfo
...
object ContactInfoDBHandler {
    ...
    fun all(row: Row?): List<ContactInfo> = try {
        if (row != null) {
    ...
    fun one(row: Row?): Optional<ContactInfo> = try {
        if (row != null) {
            val json = row.get(0, String::class.java)
            if (json != null) {
                Optional.ofNullable(
                    jsonHandler.decodeFromString<List<ContactInfo>>("""$json""").firstOrNull()
                )
            } else {
                Optional.empty()
            }
